...
- Semantic Mesh Stage F2B explicit API opt-in writer wiring: `POST /memory/add` принимает top-level transport control `relation_contract_version` только вместе с exact `strict: true`; отсутствие поля сохраняет existing permissive strict route и не передаёт новый keyword, exact `semantic_relation.v1` conditionally forwards в F2A до записи, unsupported/non-string/`null` versions возвращают stable `400 unsupported_contract_version;field=contract_version`, а opt-in вне strict mode возвращает `400 relation_contract_requires_strict_mode;field=relation_contract_version` до legacy text write; accepted relations остаются в existing `metadata.concept_core.relations`, transport field не сохраняется, outer success response shape и `/concept-core/status` unchanged, invalid transport/relation payloads не пишут blocks, target lookup, reverse edges, storage/schema migration/backfill, Semantic Mesh read semantics, planner/dispatch/replay и default requests without opt-in не менялись
- Semantic Mesh Stage F3A pure stored-relation compatibility classifier: новый standalone module `ai_os/semantic_relation_compatibility.py` классифицирует уже загруженный relation list как `empty`, `compatible_v1_shape`, `legacy_unversioned`, `mixed_versioning`, `unsupported_version`, `invalid_v1` или `malformed_relations`, для exact-v1 lists переиспользует F1 validator и возвращает только aggregate counts плюс safe `code/index/field`; `compatible_v1_shape` не доказывает writer provenance, поэтому report всегда указывает `writer_validation_provenance=not_available` и `target_lookup_performed=false`; classifier не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage, не выполняет I/O, lookup, reverse edges, migration/backfill/schema mutation или input mutation
- Semantic Mesh Stage F3B pure aggregate compatibility inventory: новый standalone module `ai_os/semantic_relation_compatibility_inventory.py` принимает только exact list caller-supplied `SemanticRelationCompatibilityReport`, валидирует report type/status/non-negative counts/aggregate и status-specific count consistency/issue invariants и возвращает deterministic fixed-taxonomy status counts плюс aggregate relation и issue-report counts; публичный value-object не позволяет доказать происхождение report, поэтому inventory всегда указывает `report_generation_provenance=not_available` и `writer_validation_provenance=not_available`; inventory aggregate-only, не принимает и не раскрывает memory blocks, Concept Core metadata, source IDs, relation payloads, targets или evidence refs, не выводит readiness/migration/enforcement decision, указывает `target_lookup_performed=false`, не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage и не выполняет storage read, I/O, lookup, reverse edges, migration/backfill/schema mutation или input mutation
- Semantic Mesh Stage F3C pure direct-input compatibility batch inventory: новый standalone module `ai_os/semantic_relation_compatibility_batch.py` принимает exact list exact frozen `SemanticRelationCompatibilityInput` с caller-supplied `source_concept_id` и уже загруженными `relations`, напрямую вызывает F3A один раз для каждого input и передаёт только созданные reports существующему F3B builder; aggregate-only output содержит fixed-taxonomy counts, `report_generation_provenance=direct_f3a_classifier`, `writer_validation_provenance=not_available` и `target_lookup_performed=false`, причём direct marker доказывает только внутрипроцессный call path, но не origin/authenticity/freshness/completeness/uniqueness/storage ownership или writer validation входов; duplicate items считаются отдельно без source-ID deduplication, output не удерживает и не раскрывает source IDs, relation payloads, reports, targets или evidence refs, не выводит readiness/migration/enforcement decision, не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage и не выполняет storage read, I/O, target lookup, reverse edges, migration/backfill/schema mutation или input mutation
- Semantic Mesh Stage F3D pure direct-batch aggregate compatibility comparison: новый standalone module `ai_os/semantic_relation_compatibility_comparison.py` принимает exact caller-supplied `baseline_inputs` и `candidate_inputs`, вызывает existing F3C builder ровно один раз для каждого batch с сохранением direct batch/list/item identity и exact returned inventory identity, затем выдаёт aggregate-only baseline/candidate counts и signed `candidate_minus_baseline` deltas для полной fixed taxonomy и всех aggregate counts; call-path provenance не доказывает origin/authenticity/freshness/completeness/uniqueness/storage ownership или writer validation, `writer_validation_provenance=not_available`, source matching/deduplication/pairing/relation alignment/target lookup и readiness/migration/enforcement interpretation не выполняются; module не удерживает relation payloads или source IDs, не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage и не выполняет storage read, I/O, reverse edges, migration/backfill/schema mutation или input mutation
- Semantic Mesh Stage F3E pure aggregate compatibility-comparison inventory: новый standalone module `ai_os/semantic_relation_compatibility_comparison_inventory.py` принимает exact list exact caller-supplied F3D `SemanticRelationCompatibilityComparison`, валидирует nested F3C inventory types, canonical taxonomy, non-negative aggregate counts, report/status/relation/issue и status-derived minimum-count invariants, а также каждый signed `candidate_minus_baseline` delta, после чего возвращает deterministic summed baseline/candidate/delta counts; F3E не вызывает F3D повторно, duplicate comparisons считаются независимо, order не влияет на результат, cancellation остаётся только арифметикой без improvement/regression/trend/readiness/migration/enforcement meaning; public F3D objects не доказывают generation provenance, поэтому output указывает `comparison_generation_provenance=not_available`, не удерживает individual comparisons/inventories/source IDs/relation payloads, не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage и не выполняет storage read, matching/deduplication/pairing/alignment, target lookup, reverse edges, I/O, migration/backfill/schema mutation или input mutation
- Semantic Mesh Stage F3F pure direct comparison-batch inventory composer: новый standalone module `ai_os/semantic_relation_compatibility_comparison_batch.py` принимает exact list exact frozen `SemanticRelationCompatibilityComparisonBatchInput`, где каждый item является только explicit caller-supplied baseline/candidate pair, вызывает F3D ровно один раз для каждой пары с exact batch identity и затем F3E ровно один раз с exact generated comparison identities; aggregate-only output сохраняет exact F3E baseline/candidate/delta object identities, использует `comparison_generation_provenance=direct_f3d_comparison_for_each_input` и `comparison_inventory_generation_provenance=direct_f3e_comparison_inventory_builder`, но не доказывает input origin/authenticity/freshness/completeness/uniqueness/storage ownership/source identity/writer validation; duplicates считаются независимо, aggregate order-independent, safe nested errors получают `comparison_index/batch/index/field`, output не удерживает input pairs/comparisons/inventories/source IDs/relation payloads и не интегрирован в Semantic Mesh/API/writer/planner/runtime/storage, не выполняет storage read, inferred source matching, target lookup, reverse edges, I/O, migration/backfill/schema mutation, input mutation или policy interpretation
