...
| `031-040` | Indexing | Existing keyword/tag/recent/search surfaces and graph-summary related memory route; recovery/snapshot lineage surfaces add operational indexing context | partial | `scripts/api_server.py`; `core/retrieval_engine.py`; `core/graph_engine.py`; `development/scripts/smoke_memory_concept_read_visibility.py` | accepted memory visibility smoke | semantic/vector mesh remains future design work, not current implementation |
...
| `061-070` | GPT context formation | `CognitiveLoop.plan()` produces top-level `concept_core_advisory`; persisted/status/run-shaped serialization filters the advisory | partial | `ai_os/cognitive_loop.py`; `development/scripts/smoke_concept_core_advisory_consumers.py`; `docs/developer/GPTMEAI_CONCEPT_CORE.md` | `smoke_concept_core_advisory_consumers.py --json --cleanup` | current system is control-plane/planner oriented; advisory is not injected into planner task metadata |
