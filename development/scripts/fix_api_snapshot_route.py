...
if "from datetime import datetime, UTC" not in text:
    text = text.replace(
        "import json\nfrom pathlib import Path",
...
if "from scripts.runtime_store import load_runtime" not in text:
    text = text.replace(
...
anchor = '        self._send_json({"error": "not found"}, status=404)'

if 'if path == "/snapshot/export":' not in text:
    text = text.replace(anchor, route_block + "\n\n" + anchor, 1)

if '"POST /snapshot/export"' not in text:
    text = text.replace(
...
print("api_server snapshot route added")